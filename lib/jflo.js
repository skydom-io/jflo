...
module.exports = (function() {
    ...
    function _rootModule(m) {
        m = m || module;
        return m.parent ? _rootModule(m.parent) : m;
    }

    JFlo.prototype.include = function(patterns) {
        if (typeof patterns == 'function') {
            // The module has been require'd already; we're ready to call the factory

            patterns(this);
            return;
        }
        var caller_dir = path.dirname(require.main.filename);
        var self = this;
        // Ensure it's an array (of string patterns)
        patterns = [].concat(patterns)
            .map(function(pattern) { return path.resolve(caller_dir, pattern)});

        var paths = globby.sync(patterns);
        paths.forEach(function(name) {
            try {
                var plugin_path = name;
    ...
    /**
     * Returns the flow alias for a custom host application
     */
    JFlo.prototype.getFlowCLIAlias = function(flow_name) {
        var app_name = (this.appConfig || {}).binary_name,
            flow_path = (this.appConfig || {}).flow_path,
            flow_alias = (flow_name.indexOf(flow_path) == 0) ? flow_name.replace(flow_path + ".", "") : flow_name;
        return app_name + ' ' + flow_alias;
