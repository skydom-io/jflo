 ...
 * @param config
 */
function run(jflo, config) {

    config = config || {};
    var exeName = config.binary_name = config.binary_name || 'jflo';
    jflo.appConfig = config;
    ...
    if (!flow_class.length) {
        return helper.print_help(jflo, null, "", config);
    }

    if (config.flow_path) {
        // Only expose the host application's specific flows
        flow_class = [config.flow_path, flow_class].join('.');
    }

    var Flow = jflo.flow(flow_class);

    if (argv.help || !Flow) {
        helper.print_help(jflo, Flow, flow_class, config);
        return;
    }
    ...
    if (typeof Flow == 'function' && Flow._meta) {
        ...
        var nameless_param_offset =
            Flow._meta.name.split('.').length
            -((config.flow_path || "").length ? config.flow_path.split('.').length : 0);

        explicit_parameters._ = argv._.slice(nameless_param_offset);
        ...
        console.error("Usage: " + [exeName].concat(plugpath).join(' ') + ' '
        + Object.keys(Flow).join('|') + ' [options]');
    }
    else {
        console.error("Usage: " + exeName + " [flow [subflow]] [options]");
        return;
    }
}
