...
function getCommonCLIParams(pcontext) {
    pcontext = pcontext || {};
    return {
        id: {
            params: ['<value>'],
        ...
        formatter: {
            params: ['<name>'],
            desc: 'Output stream formatter (' + (pcontext.formatters || []).join(' | ') + ')'
        },
        parser: {
            params: ['<name>'],
            desc: 'Input stream parser (' + (pcontext.parsers || []).join(' | ') + ')'
        }
    };
}
...
function print_help(jflo, Flow, classname, config) {
    ...
    var flow_prefix = (config.flow_path || "").split('.').join(' '),
        ...
        flow_alias = config.flow_path && (classname.indexOf(config.flow_path) == 0) ?
            classname.replace(config.flow_path, "") : classname;
    flow_alias = jflo.getFlowCLIAlias(classname);
    ...
    if (!Flow) {
        ...
        function _list(path, node, commands) {
            if (typeof node == 'function' && node._meta) {
                ...
                if (cmd.indexOf(flow_prefix) == 0) {
                    cmd = exeName + ' ' + cmd.slice(flow_prefix.length).trim();
        ...
        console.log("Command parameters for [" + flow_alias + "]:");
        var meta = (Flow._meta || {});
        ...
        var COMMON_CLI_PARAMS = getCommonCLIParams({
            formatters: Object.keys(jflo.formatters)
                .filter(function (x) {
                    return x != "$default"
                }),
            parsers: Object.keys(jflo.parsers)
                .filter(function (x) {
                    return x != "$default"
                })
        });
...
module.exports = {
    ...
    common_parameters: getCommonCLIParams()
};